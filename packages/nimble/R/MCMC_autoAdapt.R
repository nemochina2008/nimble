...
autoAdapt <- function(Rmodel,
                      ...
                      makePlots = FALSE,
                      round = TRUE ) {
    if(autoIt < 10000) stop('Minimum auto-Adapting iterations is 10,000')
    control <- list(niter=autoIt, setSeed=setSeed, verbose=verbose, makePlots=makePlots)
    ab <- autoAdaptClass(Rmodel, control)
    if(!'auto' %in% run) run <- c(run, 'auto')  ## always use 'autoAdapt' routine
    ab$run(run)
    abList <- list(ab)
...
autoAdaptModel <- setRefClass(
    ...
    methods = list(
        ...
        createGroups = function(listOfAdapts = list()) {
            ...
            if(any(unlist(listOfAdapts) %in% scalarNodeVectorDisc)) stop('cannot put block sampler on discrete-valued model nodes')
        ...
        run = function(runList) {
            ...
            for(i in seq_along(runList)) {
                ...
                switch(type,
                       ...
                       all =     { confList <- list(createConfFromGroups(abModel$nodeGroupAllAdapted))
                       ...
                       Adapts =  { confList <- list(createConfFromGroups(abModel$createGroups(runListElement)))
                                   name <- if(runListName == '') 'customAdapts' else runListName
